...
with PdfPages('Starbuck_Stock_Trend_Figures.pdf') as pdf:

    #Daily closing price 
    plotFigures(df_tickername[['close']], f'{ticker_name} Closing Price Trend', 'Time (in days)', 'Closing Price', f'{ticker_name}_Daily_Closing_Price')
    
    #Daily volume
    plotFigures(df_tickername[['volume']], f'{ticker_name} Volume Trend','Time (in days)','Volume',f'{ticker_name}_Daily_Volume')
    
    #Histograms
    titles = [f'{ticker_name} Opening Prices',f'{ticker_name} High Prices',f'{ticker_name} Low Prices',f'{ticker_name} Closing Prices',f'{ticker_name} Volume']
    ...
    plotFigures(df_tickername['Daily_Return'],f'{ticker_name} Daily Return','Time (in days)','Daily Return',f'{ticker_name}_Daily_Return')
...
#    plt.savefig(f_getFilePath(f'reports\\figures\\{ticker_name}_Daily_Return_HistKDE.png'))
    ...
    plotFigures(df_cr,f'{ticker_name} Cumulative Returns','Time (in days)','Cumulative Return',f'{ticker_name}_Cumulative_Return')
    ...
    plotFigures(df_tickername[['close', 'MA for 10 days', 'MA for 20 days', 'MA for 50 days']],f'{ticker_name} Moving Average','Time (in days)','Closing Price',f'{ticker_name}_Moving_Average')
...
#    plt.savefig(f_getFilePath(f'reports\\figures\\{ticker_name}_Autocorrelation_Plot.png'))
    ...
    #Volatility
    plotFigures(df_tickername['Volatility'],f'{ticker_name} Volatility','Time (in days)','Historical Volatility',f'{ticker_name}_Volatility')
    
    #Risk Analysis
    expected_return = df_tickername['Daily_Return'].mean()
    total_return = (df_tickername['Daily_Return'].iloc[-1] - df_tickername['Daily_Return'].iloc[1])/df_tickername['Daily_Return'].iloc[1]
    ...
    plt.ylabel('Risk')
    plt.annotate(f'{ticker_name}', xy=(x, y), xytext=(50, 50), textcoords='offset points', ha='right', va='bottom', 
