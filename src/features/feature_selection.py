...
def f_featureSelection(train_x, train_y):
    ...
    print("Performing Scaling and Factor Analysis...")

#    # Pipe flow is :
#    # Scaling the data -> PCA(Dimension reduction to two) 
#    feature_pipe = Pipeline([("minmax", preprocessing.MinMaxScaler())], verbose=True)
    num_factors = 10
    # Pipe flow is :
    # Scaling the data -> PCA(Dimension reduction to two) 
    feature_pipe = Pipeline([('std', preprocessing.MinMaxScaler()), ('fa', FactorAnalyzer(n_factors = num_factors,rotation='varimax'))], verbose = True)
